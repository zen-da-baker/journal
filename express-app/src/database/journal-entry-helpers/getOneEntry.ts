...
/* 
    This helper function validates a user token and then sends back a single journal entry that 
    was selected. The response body of a successful request is the entry and a message.
*/
export async function getOneEntry( request: any, response: any ) {
    ...
    if ( entryId === null ) {
        ...
        return response.status( 400 ).json({
            msg: "The needed details are missing."
        })
    ...
    // Now that the user is verified, get the requested entry
    ...
    if ( entry === null ) {
        ...
        return response.status( 400 ).json({
            msg: "The journal entry was not found."
        })
