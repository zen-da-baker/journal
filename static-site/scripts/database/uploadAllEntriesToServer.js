...
export async function uploadAllEntriesToServer() {
    ...
    let dreamEntriesCount = 0;
    let dayEntriesJson = localStorage.getItem("bytesized-journal-day-entries-list");
    let dreamEntriesJson = localStorage.getItem("bytesized-journal-dreams-entries-list");
    if (dayEntriesJson === null) {
        dayEntriesCount = 0;
    }
    else {
        let dayEntriesList = JSON.parse(dayEntriesJson);
    ...
    if (dreamEntriesJson === null) {
        ...
        let dreamEntriesList = JSON.parse(dreamEntriesJson);
    ...
    if (database !== null) {
        ...
        let dayRequest = dayObjectStore.count();
        let dreamRequest = dreamObjectStore.count();
        ...
        dayRequest.onsuccess = async () => {
            ...
            if (dayEntriesCount !== 0) {
                ...
                let dayEntriesList = dayRequest.result;
                const requestBody = {
                ...
                const initialResponse = await fetch("/settings/all-entries", requestOptions);
        ...
        dreamRequest.onsuccess = async () => {
            ...
            if (dreamEntriesCount !== 0) {
                ...
                let dreamEntriesList = dreamRequest.result;
                const requestBody = {
